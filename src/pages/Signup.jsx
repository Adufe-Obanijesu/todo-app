...
const Signup = () => {
        ...
        uploadBytes(profilePic, image)
          .then(() => {
            getDownloadURL(profilePic)
              ...
              .catch((err) => {
                console.log("cant get download link", err);
                setLoading(false);
              });
          })
          .catch((err) => {
  ...
  return (
    <main className="flex items-center justify-center  bg-cyan-500 h-screen">
      <div className="inline shadow bg-white rounded px-6 py-10 w-1/3">
        ...
        <form onSubmit={(e) => signup(e)}>
          ...
          <button
            ...
            type="submit"
          >
            Sign up <ClipLoader size={20} loading={loading} className="mt-1 ml-2" cssOverride={override} />
          </button>
        </form>
        <p className="mt-2">
          <span className="text-gray-400 text-sm">Already have an account?</span> <Link to="/login" className="text-sm text-cyan-500">Log in</Link>
        </p>
      </div>
    </main>
  );
